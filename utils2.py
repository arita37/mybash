...
def bike_get_mask_wheel_v1(img_dir='imgs/bik5.png', verbose=0):
    ...
    circles = cv2.HoughCircles(blur, cv2.HOUGH_GRADIENT, dp=1, minDist=30, param1=70, param2=25,
                               minRadius=int(min(gray.shape) / 10), maxRadius=int(min(gray.shape) / 5))
...
def bike_get_mask_bike(img_dir='imgs/bik5.png', points=None, labels=None, dirout="", method="sam01",
...
def image_invert_colors(img, invert_only_dark_bg=1):
    ...
    if image.shape[-1] == 4:
        ...
        # Split the image into its 3 channels (R, G, and B)
